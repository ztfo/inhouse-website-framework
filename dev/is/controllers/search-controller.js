...
.controller('searchController', ['$rootScope', '$timeout', '$scope', 'inhouseApi', '$routeParams', '$location', '$window', 'listing', function($rootScope, $timeout, $scope, inhouseApi, $routeParams, $location, $window, listing) {
  ...
  $scope.searchMLS = function() {
    ...
    if(Object.keys($scope.filters).length > 0) {
      ...
      listing.search(params).success(function(response) {
        var listings = _.map(_.get(response, 'data.listings'), function(data) {
          return listing.hydrate(data);
        });

        if($scope.noWipe) {
          _.map(listings, function(newListing) {
            $scope.listings.push(newListing);
          });
        } else {
          $scope.listings = listings;
        }
        $scope.totalListings = _.get(response, 'data.pagination.last_count');
        $scope.displayListings = _.get(response, 'data.pagination.page_size');
